...
처리해야 하는 사건들이 3개 있고 그 사건들이 발생된 위치를 순서대로 (3, 5), (5, 5), (2, 3)이라고 하자.
...
각 위치는 동서방향 도로 번호를 나 타내는 정수와 남북방향 도로 번호를 나타내는 정수로 주어지며
...
void DFS(int w, int p1, int p2, int dist)
{
    if (distSum <= dist) return; //가지치기
    if(w>=W)
    {
        distSum = dist;
        return;
    }
    DFS(w + 1, w, p2, dist + (abs(pos[p1].r - pos[w].r) + abs(pos[p1].c - pos[w].c)));
    DFS(w + 1, p1, w, dist + (abs(pos[p2].r - pos[w].r) + abs(pos[p2].c - pos[w].c)));
}
